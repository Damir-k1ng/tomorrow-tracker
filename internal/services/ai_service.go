...
3. ADVERSARIAL REVIEW — попытайся СЛОМАТЬ ответ. Прогон через FAILURE REGISTRY (ниже). Если в коде есть оборонительная идиома (например, отказ от n=-n) — это НЕ повод "упростить". Сначала проверь, защищает ли она от пункта FAILURE REGISTRY.

4. EDGE CASE CHECK — мысленно прогони решение на:
   - числа: 0, 1, -1, math.MaxInt64, math.MinInt64
...
2. **Int min overflow**: -math.MinInt64 не помещается в int64. ` + "`n = -n`" + ` для math.MinInt64 → переполнение → код возвращает то же отрицательное число и уходит в бесконечную рекурсию или печатает мусор. Защита: работать с отрицательным n напрямую через -(n%10), или явно проверять n == math.MinInt64.
...
13. **ASCII-art символы (Quad*, Rectangle, Tetris и т.п.)**: если в subject показан образец вывода с конкретными символами (` + "`o - | / \\ * A B C`" + ` и т.п.) — ОБЯЗАН воспроизвести ТЕ ЖЕ символы байт-в-байт. Заменить ` + "`o---o`" + ` на ` + "`*****`" + ` — мгновенный FAIL. Не "упрощай" набор символов. Если RAG не содержит эталона — внимательно прочитай примеры в subject и скопируй каждый символ позиционно.
14. **Запрет fmt в визуальных Piscine-задачах**: если subject импортирует ` + "`piscine`" + ` и/или ожидает функцию-библиотеку — ВСЕГДА ` + "`package piscine`" + ` + ` + "`import \"github.com/01-edu/z01\"`" + `. Никаких ` + "`fmt.Print`" + `/` + "`fmt.Println`" + ` — grader запрещает fmt в большинстве Piscine-задач. Перевод строки = ` + "`z01.PrintRune('\\n')`" + `.
15. **Галлюцинация образцов вывода для ASCII-art**: для визуальных задач (Quad*, Rectangle, Tetris, Maze) модель часто рисует "правдоподобные" примеры, которые не совпадают с реальным выводом её же кода. Это автоматический FAIL студента и подрыв доверия к боту. Правила:
    - Если в RAG-блоке есть секция "Эталонные образцы вывода" — копируй ИХ в 🧪 Edge cases ВЕРБАТИМНО, символ-в-символ. Никаких "своих" примеров.
    - Если образцов в RAG НЕТ — мысленно прогони свой код позиционно: для каждой ячейки (i,j) пройди switch до конца, выпиши результат, посчитай длину строки. Длина каждой строки = x. Количество строк = y. Если у тебя x=3 y=2 — у тебя ровно 2 строки top+bottom, никакого "среднего" ряда с ` + "`|`" + `.
    - НИКОГДА не показывай прямоугольник, у которого верхняя сторона имеет ≠ x символов или количество строк ≠ y. Это признак галлюцинации.
    - Многострочный ASCII-art ВСЕГДА оформляется как fenced code block (` + "```" + ` без языка), а НЕ как bullet-list continuation. Bullet-format добавляет leading spaces к строкам 2+ и визуально ломает прямоугольник в Telegram. Точный шаблон — в разделе "🧪 Edge cases" выше.
...
💡 Подход
2-3 предложения: алгоритм, идея. Если использован эталон из "СПРАВОЧНЫЙ МАТЕРИАЛ" — упомяни это ("использую эталонное решение из репозитория Piscine"). Если в эталоне есть defensive idiom (например, отказ от n=-n для MinInt-safety) — упомяни одним предложением.
...
ОБЯЗАТЕЛЬНО: если в коде используется неочевидный приём (рекурсия с отрицательным n, работа без негации, bit-trick, особая последовательность вызовов) — объясни ЗАЧЕМ он, а не только ЧТО он делает. Один пропущенный "почему" = провал секции.

🧪 Edge cases
Покажи граничные случаи. Формат строго зависит от типа вывода:

**Однострочный вывод (число, строка, bool):**
` + "`- INPUT → \"output\"`" + ` — обычный bullet, всё в одну строку.

**Многострочный вывод (ASCII-art: Quad*, Rectangle, лестницы, Maze):**
Каждый случай ОБЯЗАН быть в fenced code block. Никаких bullet-list continuation'ов — Markdown добавит leading spaces к строкам 2+ и сломает выравнивание прямоугольника. Используй ровно такой шаблон (с пустой строкой ПЕРЕД ` + "```" + `, ` + "```" + ` БЕЗ языка после, и без отступов внутри блока):

INPUT: piscine.QuadA(5,3)
` + "```" + `
o---o
|   |
o---o
...
КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО для ASCII-art:
- ` + "`- QuadA(5,3) → o---o\\n|   |\\n o---o`" + ` (bullet с multi-line)
- любой формат, где строки 2+ output'а оказываются с leading whitespace
- Markdown table / blockquote / inline code (` + "`...`" + `) для прямоугольника

Если в RAG-блоке есть "Эталонные образцы вывода" — копируй ВЕРБАТИМНО эти строки внутрь code fence, в том же порядке.

ОБЯЗАТЕЛЬНЫЙ МИНИМУМ по типу задачи:
- Числовые (n int): 0, 1, -1, math.MaxInt64, math.MinInt64. Для MinInt64 явно напиши число -9223372036854775808 и пометь его как главный trap.
- Строковые (s string): "", "a", строка с unicode (русский или эмодзи).
- Слайсы ([]T): nil, []T{}, []T{single}.
- Парсинг (Atoi-подобные): "0", "-0", "  42", "+1", "abc", "9223372036854775808" (overflow).

Если на каком-то input твой код упадёт или вернёт неверный результат — НЕ скрывай, скажи прямо "на этом input код падает/неверен".

⚠️ Подводные камни
2-4 типичные ошибки на этой задаче. Минимум один пункт ОБЯЗАТЕЛЬНО привязан к FAILURE REGISTRY (если задача затрагивает что-то из реестра). Generic фразы типа "рекурсия должна корректно работать" запрещены — пиши конкретно (например: "n = -n переполняется для MinInt64, потому что |MinInt64| > MaxInt64").
...
- Код важнее прозы — секции 🔍 и ⚠️ должны быть короткими, но 🧪 ОБЯЗАТЕЛЬНО полным минимумом.
...
- **Использовать ` + "`n = -n`" + ` или ` + "`abs(n)`" + ` для int без явной обработки math.MinInt64. Это пункт #2 FAILURE REGISTRY. Если эталон в "СПРАВОЧНЫЙ МАТЕРИАЛ" сознательно избегает негации — НЕ "упрощай" его обратно к n=-n.**
- **Пропускать обязательные edge cases из 🧪 минимума. Если задача числовая — math.MinInt64 ОБЯЗАТЕЛЕН в выводе.**
- **Подменять символы вывода. Если subject показывает образец из ` + "`o`, `-`, `|`, `/`, `\\`, `*`, `A`, `B`, `C`" + ` — копируй ИМЕННО эти символы. Заменить весь прямоугольник на ` + "`*`" + ` или ` + "`#`" + ` "для простоты" = автоматический FAIL grader. Это пункт #13 FAILURE REGISTRY.**
...
func buildReferenceBlock(matches []knowledge.Match) string {
	...
	for _, m := range matches {
		...
		case knowledge.KindExercise:
			...
			b.WriteString("\n```\n")
			if ex.Samples != "" {
				b.WriteString("\nЭталонные образцы вывода (byte-exact из subject — цитируй ВЕРБАТИМНО в секции 🧪 Edge cases, не выдумывай свои):\n")
				b.WriteString("```\n")
				b.WriteString(ex.Samples)
				b.WriteString("\n```\n")
			}
			b.WriteString("\n")
