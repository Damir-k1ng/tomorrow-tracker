...
func buildReferenceBlock(matches []knowledge.Match) string {
	...
	b.WriteString("1. Если выше есть упражнение Piscine — используй его эталонное решение ВЕРБАТИМНО в секции '✅ Решение'. Не меняй package, не добавляй своих функций, не упрощай — это код который принимает grader.\n")
	b.WriteString("2. Код в `package piscine` означает: студент кладёт эти функции в файл с `package piscine` (без func main). Это форма для сдачи в grader.\n")
	b.WriteString("3. Код в `package main` означает: это стандалонная программа (например, читает аргументы, печатает результат). Студент сдаёт файл целиком как `package main`.\n")
	b.WriteString("4. Если только концепции — используй их как фундамент объяснения.")
	return b.String()
}
